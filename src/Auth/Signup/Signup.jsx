...
export const Signup = () => {
    ...
    const InputField = ({ label, type, placeholder, className, value, onChange, dropdownIcon, options }) => (
        <div>
            <label htmlFor="" className='label_style mt-2'>{label}</label>
            {type === 'select' ? (
                <div className="custom-dropdown">
                    <select className={`style_input ${className}`} value={value} onChange={onChange}>
                ...
                <div className='position-relative'>
                    <input type={type} placeholder={placeholder} className={`style_i_input ${className}`} value={value} onChange={onChange} />
                    {dropdownIcon && (
    ...
    const renderPasswordInput = (label, placeholder, showPassword, togglePasswordVisibility) => (
        <div>
            <label htmlFor="" className='label_style mt-2'>{label}</label>
            <div className='position-relative'>
                <input type={showPassword ? 'text' : 'password'} placeholder={placeholder} className='style_i_input' />
                <span className='toggle_input_password' onClick={togglePasswordVisibility}>
